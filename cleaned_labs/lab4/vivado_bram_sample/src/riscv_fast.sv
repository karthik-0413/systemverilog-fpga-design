...
logic [31:0]         registerFile [31:0];
 ...
 // The five immediate formats, see RiscV reference (link above), Fig. 2.4 p. 12
   ...
   assign Iimm = {{20{instR[31]}}, instR[30:20]};
   assign Simm = {{20{instR[31]}}, instR[30:25], instR[11:7]};
  ...
  assign aluPlus = aluIn1 + aluIn2; // ??
   ...
   assign aluOut =
     (funct3Is[0]  ? (isALUreg & instR[30] ? aluMinus[31:0] : aluPlus)  : 32'b0) |  // add, sum
     (funct3Is[1]  ? leftshift                                          : 32'b0) |  // sll
     (funct3Is[2]  ? (LT ? 32'b1 : 32'b0)                               : 32'b0) |  // slt
     (funct3Is[3]  ? (LTU ? 32'b1 : 32'b0)                              : 32'b0) |  // sltu
   ...
   assign PCplusImm = (isBranch) ? PC + Bimm : (isAUIPC) ? PC + Uimm : PC + Jimm; // ??

   logic jumpToPCplusImm; // Condition to jump to PCplusImm
   assign jumpToPCplusImm = (isBranch && branch_condition) || (isJAL); // ??
   // Next PC selection (handle PCplus4, PCplusImm, JALR)
   assign PC_next = (isJALR) ? aluOut : (jumpToPCplusImm) ? PCplusImm : PCplus4; // (aluIn1 + Iimm) && ~1'b1 : PCplus4; // ??
   ...
   always @(posedge clk) begin
      if(!reset) begin
         ...
         // Update PC
         if ((state == EXECUTE && !(needToWait)) || state == WAIT_MEM) begin
            PC <= PC_next;
         end
